...
function btn(text, id, active = false, small = false) {
    ...
    const html = `
      <li id="${id}-btn" style="margin-bottom:6px; list-style:none;">
        <a href="javascript:void(0)" class="_pkhvgz8" data-path="${id.replace(/-/g, '.')}" style="
          display:flex; align-items:center; background-color:${bg};
          font-size:${size}; padding:${pad}; cursor:pointer;
          user-select:none; text-decoration:none; color:#21242c;">
          <span>${text}</span>
        </a>
      </li>`;
...
function delayInput(val) {
    ...
    const existing = document.getElementById("auto-delay");
    ...
    wrap.innerHTML = `
      <label for="delayInput" style="
        font-size:14px; color:#21242c; user-select:none; flex-grow:1; cursor:pointer;">
        Delay (s):
      </label>
      <input id="delayInput" type="number" min="0.5" step="0.1" value="${val}" placeholder="Delay (4s)" style="
        width:70px;
        padding:4px 8px;
        font-size:14px;
...
function addRgbCss() {
    ...
    style.textContent = `
      ...
      .rgb-anim {
        animation: rgbCycle 10s linear infinite;
      }
    `;
...
function buildUI() {
    ...
    const btns = [{
            text: t("Sobrescrever", "Overwrite"),
            path: "exp.Overwrite.enabled"
        },
        {
            text: t("Automatizar", "Automate"),
            path: "exp.Overwrite.automate",
            small: true
        },
        {
            text: t("Mostrar Resposta", "Show Answer"),
            path: "exp.showAnswer"
        },
        {
            text: t("Falso Vídeo", "Video Spoof"),
            path: "exp.videoSpoof"
        },
        {
            text: t("Parar Temporizador", "Stop Timer"),
            path: "exp.stopTimer"
        },
        {
            text: t("RGB", "RGB"),
    ...
    btns.forEach(({
        ...
        if (path === "exp.Overwrite.automate") {
            setTimeout(() => delayInput(exp.Overwrite.delay), 10);
        }
    });
